/*Bob is a librarian. 
He requires a system that can enable him to add new books, delete any existing book, search for the availability of books, and print all the books. 
The system should also be able to store the information about the books in a file and read from the file when the program is run. 
Create a structure called books and store the relevant information and create separate user-defined functions for performing the above operations. 
Write a menu-driven program to get a choice from the librarian. Based on his choice, the corresponding functions should be invoked, and the structure should be updated. 
